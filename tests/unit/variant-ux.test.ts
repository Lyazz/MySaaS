...
describe('variant-ux', () => {
    ...
    it('getPreferredInitialSelection prefers an in-stock active variant', () => {
        const selection = getPreferredInitialSelection(product)
        expect(selection).toEqual({ opt_size: 's', opt_color: 'blue' })
    })

    it('findBestVariantForSelection returns an in-stock match when possible', () => {
        const variant = findBestVariantForSelection({ product, selectedOptions: { opt_size: 's', opt_color: 'red' } })
        // Only red is out of stock here, so best should be that (no in-stock match for red)
        expect(variant?.id).toBe('v_s_red')

        const variant2 = findBestVariantForSelection({ product, selectedOptions: { opt_size: 's', opt_color: 'blue' } })
