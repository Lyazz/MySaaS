  ...
  <NuxtLayout name="super-admin">
    <div class="space-y-6">
      ...
      <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl overflow-hidden">
        <div v-if="loading" class="p-8 text-center text-gray-400">Loading tenants...</div>
        <div v-else-if="filteredTenants.length === 0" class="p-8 text-center text-gray-400">No tenants found</div>
        <table v-else class="w-full">
          <thead class="bg-slate-900/50 border-b border-slate-700">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300">Products</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-gray-300">Orders</th>
          ...
          <tbody class="divide-y divide-slate-700">
            <tr v-for="tenant in filteredTenants" :key="tenant.id" class="hover:bg-slate-900/30 transition-colors">
              ...
              <td class="px-6 py-4 text-gray-300">{{ tenant._count?.users || 0 }}</td>
              <td class="px-6 py-4 text-gray-300">{{ tenant._count?.products || 0 }}</td>
              <td class="px-6 py-4 text-gray-300">{{ tenant._count?.orders || 0 }}</td>
              <td class="px-6 py-4 text-gray-300 text-sm">{{ formatDate(tenant.createdAt) }}</td>
              <td class="px-6 py-4">
                <div class="flex justify-end space-x-2">
                  ...
                  <button v-if="!tenant.isSuspended" @click="suspendTenant(tenant)" 
                  ...
                  <button v-else @click="unsuspendTenant(tenant)" 
    ...
    <div v-if="showCreateModal || showEditModal" 
      ...
      <div class="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-md">
        ...
        <div v-if="error" class="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-300 text-sm">
          {{ error }}
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="formData.slug" type="text" required :disabled="!!showEditModal"
              ...
              placeholder="acme" />
            <p class="text-xs text-gray-400 mt-1">Will be accessible at {{ formData.slug || 'slug' }}.localhost:3000</p>
          </div>

          <div v-if="!showEditModal">
            <label class="block text-sm font-medium text-gray-300 mb-2">Owner Email</label>
            <input v-model="formData.ownerEmail" type="email" required
              class="w-full px-4 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
          ...
          <div v-if="!showEditModal">
            ...
            <input v-model="formData.ownerPassword" type="password" required minlength="8"
              class="w-full px-4 py-2 bg-slate-900/50 border border-slate-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500"
              placeholder="••••••••" />
          </div>
