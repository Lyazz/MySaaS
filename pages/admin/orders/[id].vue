<template>
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <nav class="flex mb-6" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          <li class="inline-flex items-center">
            <NuxtLink to="/admin/orders" class="text-gray-700 hover:text-indigo-600">
              Orders
            </NuxtLink>
          </li>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">{{ formatDate(order.createdAt) }}</p>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">
                <a :href="`tel:${order.customerPhone}`" class="text-indigo-600 hover:text-indigo-800">
                  {{ order.customerPhone }}
                </a>
              </p>
            </div>
            <div>
              ...
              <p class="mt-1 text-sm text-gray-900">{{ order.customerAddress }}</p>
            </div>
          </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="item in order.items" :key="item.id">
                  <td class="px-4 py-3 text-sm text-gray-900">{{ item.product?.title || 'Product' }}</td>
                  <td class="px-4 py-3 text-sm text-gray-900">${{ Number(item.price).toFixed(2) }}</td>
                  <td class="px-4 py-3 text-sm text-gray-900">{{ item.quantity }}</td>
                  <td class="px-4 py-3 text-sm text-gray-900 text-right">${{ (Number(item.price) * item.quantity).toFixed(2) }}</td>
                </tr>
              </tbody>
              <tfoot>
                <tr>
                  <td colspan="3" class="px-4 py-3 text-sm font-semibold text-gray-900 text-right">Total</td>
                  <td class="px-4 py-3 text-sm font-semibold text-indigo-600 text-right">${{ Number(order.totalAmount).toFixed(2) }}</td>
                </tr>
              </tfoot>
            </table>
          </div>
      ...
      <div v-else class="bg-white rounded-lg shadow p-12 text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">This order may not exist or you don't have permission to view it.</p>
        <div class="mt-6">
          <NuxtLink to="/admin/orders" class="text-indigo-600 hover:text-indigo-800">
            Back to Orders
          </NuxtLink>
        </div>
      </div>
