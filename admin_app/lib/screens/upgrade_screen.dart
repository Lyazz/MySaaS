...
class _UpgradeScreenState extends ConsumerState<UpgradeScreen> {
  ...
  Future<void> _performUpgrade() async {
    ...
    try {
      ...
      final products = await db.query('products', where: 'tenantId = ?', whereArgs: [oldTenantId]);
      final categories = await db.query('categories', where: 'tenantId = ?', whereArgs: [oldTenantId]);
      final customers = await db.query('customers', where: 'tenantId = ?', whereArgs: [oldTenantId]);
      final orders = await db.query('orders', where: 'tenantId = ?', whereArgs: [oldTenantId]);
      ...
      final newTenantName = await ref.read(authProvider.notifier).register(
      ...
      await db.transaction((txn) async {
        const tables = ['products', 'categories', 'customers', 'orders', 'sales', 'sync_queue'];
        for (final table in tables) {
          await txn.update(table, {'tenantId': newTenantId}, where: 'tenantId = ?', whereArgs: [oldTenantId]);
        }
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Upgrade to Online Tier'),
        elevation: 0,
      ),
      body: Center(
          ...
          child: ConstrainedBox(
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  const Icon(LucideIcons.cloudLightning, size: 64, color: Colors.blue),
                  ...
                  TextFormField(
                    ...
                    decoration: const InputDecoration(labelText: 'Store URL (Slug)', border: OutlineInputBorder(), prefixText: 'swekly.com/'),
                    validator: (v) => v!.isEmpty ? 'Required' : null,
                  ),
                  const SizedBox(height: 16),
                  TextFormField(
