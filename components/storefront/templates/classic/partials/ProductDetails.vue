...
<template>
    <div class="flex flex-col animate-fade-in-right space-y-8">
        <!-- Header -->
        <div>
        <h1 class="text-3xl md:text-5xl font-serif font-bold text-slate-900 tracking-tight leading-tight mb-6">
            {{ product?.title }}
        </h1>
...
<div class="text-3xl font-serif text-slate-900">
        ...
        <div
            ...
            :key="option.id"
        >
            <label class="block text-xs uppercase tracking-widest font-bold text-slate-900 mb-3">{{ option.name }}</label>
                   ...
                   <!-- Slash for unavailable / out-of-stock -->
                    <div v-if="optionValueState(option.id, value.id) !== 'available'" class="absolute inset-0 flex items-center justify-center">
                        <div
                            ...
                            :class="optionValueState(option.id, value.id) === 'out_of_stock' ? 'bg-red-500' : 'bg-slate-500'"
                        />
                    </div>
                </button>
            </div>
