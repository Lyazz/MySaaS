...
<template>
    <div class="flex flex-col animate-fade-in-right space-y-8">
        ...
        <div class="flex flex-wrap items-baseline gap-3 md:gap-4 mb-4">
            <div class="text-4xl font-sans font-bold text-[#4c1d95] font-display tracking-tight">
            {{ formatPrice(currentPrice) }}
            </div>
            <div
            v-if="originalPrice && originalPrice > currentPrice"
            class="text-lg text-slate-400 font-bold line-through decoration-2 decoration-slate-200 whitespace-nowrap"
            >
            {{ formatPrice(originalPrice) }}
            </div>
            <div v-if="originalPrice && originalPrice > currentPrice" class="px-3 py-1 bg-pink-400 text-white border-2 border-pink-300 text-xs font-black uppercase tracking-wider rounded-xl shadow-sm transform rotate-3 shrink-0">
        ...
        <!-- Urgency Banner -->
        <div class="bg-brand-100 border-4 border-brand-200 rounded-[2rem] p-4 flex items-center gap-3 shadow-sm">
            ...
            <span class="text-sm font-bold text-brand-900">
                <strong class="font-black">{{ $t('storefront.product.highDemandBadge') }}</strong> {{ $t('storefront.product.viewingRightNow', { count: 15 }) }}
            </span>
        </div>
        </div>
        ...
        >
            ...
            <!-- Dropdown Type -->
             <div v-if="option.displayType === 'dropdown'" class="relative max-w-xs">
                <select
                    ...
                    class="block w-full h-12 rounded-2xl border-4 border-purple-100 bg-white px-4 text-slate-900 font-bold focus:border-brand-500 hover:border-brand-200 transition-all duration-200 outline-none appearance-none cursor-pointer shadow-sm"
                >
	                    <option 
	                   ...
	                   <!-- Checkmark for selected state -->
	                    ...
	                    <div v-if="optionValueState(option.id, value.id) !== 'available'" class="absolute inset-0 flex items-center justify-center">
	                        <div
	                            ...
	                            :class="optionValueState(option.id, value.id) === 'out_of_stock' ? 'bg-red-500/70' : 'bg-slate-500'"
	                        />
	                    </div>
	                </button>
	            </div>
            ...
            <div v-else-if="option.displayType === 'radio'" class="space-y-2">
                 ...
                 >
	                    ...
	                    <label 
	                        ...
	                        :class="{
	                            'text-slate-400 line-through': optionValueState(option.id, value.id) !== 'available',
            ...
            <button 
                v-for="value in option.values" 
                :key="value.id"
                class="px-5 py-2.5 rounded-full text-sm font-bold border-2 transition-all duration-200 relative overflow-hidden"
                :class="[
                    optionValueState(option.id, value.id) === 'unavailable'
                        ? 'bg-slate-50 text-slate-300 border-slate-100 cursor-not-allowed decoration-slate-300'
                        : selectedOptions[option.id] === value.id
                            ? 'bg-brand-500 text-white border-brand-500 shadow-[0_4px_0_0_#7e22ce] -translate-y-1'
                            : optionValueState(option.id, value.id) === 'out_of_stock'
                                ? 'bg-white text-red-700 border-red-200 hover:border-red-300 hover:bg-red-50'
                                : 'bg-white text-slate-700 border-purple-100 hover:border-brand-300 hover:bg-purple-50 hover:-translate-y-0.5 shadow-sm'
                ]"
                :disabled="isOptionValueUnavailable(option.id, value.id)"
                :title="`${value.label} ${optionValueSuffix(option.id, value.id)}`.trim()"
                @click="setOptionIfAllowed(option.id, value.id)"
