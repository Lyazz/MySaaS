...
<template>
  <div class="flex flex-col animate-fade-in-right space-y-7" style="font-family: 'DM Sans', sans-serif">
    <!-- Title + Price -->
    <div>
      <h1 class="text-3xl md:text-4xl font-black text-stone-900 leading-tight mb-4" style="font-family: 'Fredoka', sans-serif">
        {{ product?.title }}
      </h1>

      <!-- Pricing -->
      <div class="flex flex-wrap items-baseline gap-3 mb-5">
        <span class="text-4xl font-black text-violet-700" style="font-family: 'Fredoka', sans-serif">
        ...
        >-{{ Math.round(((originalPrice - currentPrice) / originalPrice) * 100) }}%</span>
      </div>
      ...
      <!-- Urgency Banner -->
      <div class="bg-violet-50 border-3 border-violet-100 rounded-2xl p-3.5 flex items-center gap-3">
        <div class="relative flex h-3 w-3 flex-shrink-0">
          ...
          <span class="relative inline-flex rounded-full h-3 w-3 bg-violet-600" />
        </div>
        <span class="text-sm font-bold text-violet-900">
          <strong class="font-black">{{ $t('storefront.product.highDemandBadge') }}</strong>
    ...
    <div v-if="product?.options && product.options.length > 0" class="space-y-5">
      <div v-for="option in product.options" :key="option.id">
        <label class="block text-sm font-black text-stone-700 mb-2.5" style="font-family: 'Fredoka', sans-serif">
          {{ option.name }}
        </label>
        ...
        <div v-if="option.displayType === 'dropdown'" class="relative max-w-xs">
          ...
          >
            ...
            >{{ value.label }} {{ optionValueSuffix(option.id, value.id) }}</option>
          </select>
        ...
        <div v-else-if="option.displayType === 'color'" class="flex flex-wrap gap-3">
          ...
          >
            ...
            <div v-if="optionValueState(option.id, value.id) !== 'available'" class="absolute inset-0 flex items-center justify-center rounded-full overflow-hidden">
              <div
        ...
        <div v-else-if="option.displayType === 'image'" class="flex flex-wrap gap-2.5">
          ...
          >
            ...
            <span v-else class="text-xs text-stone-400 p-1 text-center">{{ value.label }}</span>
            <div
        ...
        <div v-else-if="option.displayType === 'radio'" class="space-y-2">
          <div v-for="value in option.values" :key="value.id" class="flex items-center gap-2">
            <input
              type="radio"
            ...
            >{{ value.label }} <span class="text-xs font-normal ml-1">{{ optionValueSuffix(option.id, value.id) }}</span></label>
          </div>
        </div>
        ...
        <div v-else class="flex flex-wrap gap-2">
          ...
          >
            ...
            <span v-if="optionValueState(option.id, value.id) !== 'available'" class="ml-1.5 text-[10px] font-bold">
