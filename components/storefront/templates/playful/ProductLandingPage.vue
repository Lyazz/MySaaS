...
const currentPrice = computed(() => {
    const base = currentVariant.value ? Number(currentVariant.value.price) : Number(props.product?.price || 0)
...
<template>
  <div class="bg-[#fffbf0] min-h-screen py-6 pb-28 md:pb-10 overflow-x-hidden" style="font-family: 'DM Sans', sans-serif">
    ...
    <div class="w-full mb-8 animate-fade-in-up">
      ...
      />
      <p v-else class="text-stone-500 text-lg leading-relaxed px-4">Experience premium quality with our latest collection.</p>
    </div>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      <div class="lg:grid lg:grid-cols-12 lg:gap-12 items-start">
        ...
        <!-- Details + Form -->
        <div class="lg:col-span-6 flex flex-col gap-7 lg:sticky lg:top-8 bg-white rounded-3xl border-3 border-violet-100 shadow-[0_4px_0_0_#ddd6fe] p-6 md:p-8">
          <ProductDetails
            :product="product"
    ...
    >
      ...
      >
        <div class="flex flex-col">
          ...
          <span class="font-black text-violet-700 text-xl leading-none" style="font-family: 'Fredoka', sans-serif">{{ formatPrice(currentPrice) }}</span>
        </div>
        <button
