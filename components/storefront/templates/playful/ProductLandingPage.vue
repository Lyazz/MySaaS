...
<template>
  <div class="bg-[#faf5ff] min-h-screen py-6 font-sans pb-24 md:pb-10 overflow-x-hidden w-full">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      ...
      <div class="flex flex-col gap-8">
        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-12 items-start">
            ...
            <div class="lg:col-span-6 flex flex-col gap-8 lg:sticky lg:top-8 bg-white/80 backdrop-blur-sm p-6 md:p-8 rounded-[3rem] border-4 border-purple-100 shadow-sm relative overflow-hidden">
                <!-- Decorative element -->
                <div class="absolute -top-10 -right-10 w-40 h-40 bg-pink-100 rounded-full mix-blend-multiply opacity-50 blur-xl"></div>
    ...
    <!-- Mobile Sticky Bottom Bar (Bubbled CTA) -->
    <div
      v-if="cartEnabled"
      class="fixed bottom-6 left-6 right-6 lg:hidden z-40 flex items-center justify-between p-4 bg-white/90 backdrop-blur-md rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.12)] border-2 border-purple-200"
    >
      <div class="flex flex-col">
        <span class="text-xs text-slate-500 font-bold uppercase tracking-wider">{{ storefrontContent.productForm.totalPrice }}</span>
        <span class="font-black text-[#4c1d95] text-xl font-display">{{ formatPrice(Number(product?.price || 0)) }}</span>
      </div>
       <button
        onclick="window.scrollTo({ top: 0, behavior: 'smooth' })"
        class="bg-[#fbbf24] text-amber-900 px-8 py-4 rounded-full font-black animate-pulse hover:animate-none hover:bg-amber-300 hover:-translate-y-1 shadow-[0_6px_0_0_#d97706] active:translate-y-2 active:shadow-none transition-all flex items-center gap-2"
      >
        <Icon name="lucide:arrow-up" class="w-5 h-5 stroke-[3]" />
