...
<template>
  <div class="flex flex-col animate-fade-in-right space-y-8">
    ...
    <div>
      <h1
        class="text-3xl md:text-4xl lg:text-5xl font-sans font-bold text-white tracking-tight leading-tight mb-2"
      >
        {{ product?.title }}
      </h1>

      <div
        class="flex items-center gap-2 mb-4 text-sm font-medium text-slate-400"
      >
        ...
        <span class="font-bold text-slate-200">4.9 / 5</span>
      ...
      <div class="flex flex-wrap items-baseline gap-3 md:gap-4 mb-4">
        <div class="text-4xl font-sans font-bold text-brand-500 tracking-tight">
      ...
      <!-- Urgency Banner -->
      <div
        class="bg-brand-500/10 border border-brand-100 rounded-xl p-3 flex items-center gap-3 shadow-sm"
      >
      ...
      <div v-for="option in product.options" :key="option.id">
        ...
        <div v-if="option.displayType === 'dropdown'" class="relative max-w-xs">
          <select
            ...
            "
            class="block w-full h-11 rounded-lg border border-white/10 bg-[#0b0f14] px-4 text-white focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all duration-200 outline-none appearance-none cursor-pointer shadow-sm"
          >
        ...
        >
          <button
            ...
            :class="[
              ...
              optionValueState(option.id, value.id) === 'unavailable'
                ...
                : optionValueState(option.id, value.id) === 'out_of_stock'
                  ? 'opacity-70 cursor-pointer border-white/[0.06]'
          ...
          >
            ...
            <div
              ...
              class="absolute inset-x-1 bottom-1 rounded bg-[#0b0f14]/90 text-[10px] font-semibold text-red-700 px-1 py-0.5"
            >
              Out
            </div>
          </button>
        ...
        <div v-else-if="option.displayType === 'radio'" class="space-y-2">
          ...
          >
            <input
              ...
              :disabled="isOptionValueUnavailable(option.id, value.id)"
              class="w-4 h-4 text-brand-500 border-white/15 focus:ring-brand-500 disabled:opacity-50"
              @change="setOptionIfAllowed(option.id, value.id)"
            />
            <label
              :for="`${option.id}-${value.id}`"
              class="ms-2 block text-sm font-medium text-slate-300"
              :class="{
        ...
        <div v-else class="flex flex-wrap gap-2">
          <button
            ...
            :class="[
              optionValueState(option.id, value.id) === 'unavailable'
                ? 'bg-[#06080c] text-slate-300 border-white/[0.06] cursor-not-allowed decoration-slate-300'
                : selectedOptions[option.id] === value.id
                  ? 'bg-brand-500 text-white border-brand-600 shadow-md shadow-brand-600/20'
                  : optionValueState(option.id, value.id) === 'out_of_stock'
                    ? 'bg-[#0b0f14] text-red-700 border-red-200 hover:border-red-300 hover:bg-red-50'
                    : 'bg-[#0b0f14] text-slate-300 border-white/10 hover:border-brand-300 hover:bg-white/[0.04]',
            ]"
            :disabled="isOptionValueUnavailable(option.id, value.id)"
            :title="
              `${value.label} ${optionValueSuffix(option.id, value.id)}`.trim()
    ...
    <p
      ...
      class="text-slate-400 mb-6 text-lg leading-relaxed"
    >
