...
<template>
  <div class="bg-slate-50 min-h-[calc(100vh-theme('spacing.20'))]">
    ...
    <section class="relative overflow-hidden pt-16 pb-24">
      <div class="absolute inset-0 bg-gradient-to-br from-slate-50 to-white/0 pointer-events-none"></div>
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <p class="text-xl text-slate-500 max-w-2xl mx-auto mb-10 leading-relaxed animate-slideUp" style="animation-delay: 100ms">
          Discover our premium collection of products. Quality and satisfaction guaranteed.
        </p>

        <div class="flex flex-col sm:flex-row gap-4 justify-center animate-slideUp" style="animation-delay: 200ms">
          <NuxtLink
            to="/products"
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      <div class="flex items-center justify-between mb-8">
        ...
        <NuxtLink to="/products" class="text-brand font-medium hover:opacity-80 flex items-center gap-1 group">
          View all
      ...
      <div v-else-if="featuredProducts.length === 0" class="text-center py-12 bg-white rounded-2xl shadow-sm border border-slate-100">
        <svg class="w-12 h-12 text-slate-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
        </svg>
        <p class="text-slate-500">No products available yet. Check back soon!</p>
      </div>

      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <ProductCard v-for="product in featuredProducts" :key="product.id" :product="product" class="animate-fadeIn" />
      </div>
    </section>
  </div>
</template>
