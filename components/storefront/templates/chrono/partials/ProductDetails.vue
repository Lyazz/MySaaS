...
import { getOptionValueState, type OptionValueState, type SelectedOptions } from '../../modern/variant-ux'
...
<template>
    <div class="flex flex-col space-y-7" style="animation: fadeInRight 0.6s ease forwards;">
        <!-- Title + Rating -->
        <div>
            <h1 class="text-3xl md:text-4xl lg:text-5xl font-light tracking-wide leading-tight mb-3" style="color:#E8E0D5; font-family:'Cormorant Garamond',serif;">
                {{ product?.title }}
            </h1>

            <div class="flex items-center gap-3 text-sm mb-5">
                <div class="flex" style="color:#A67C52;">
                    <Icon v-for="i in 5" :key="i" name="lucide:star" class="w-3.5 h-3.5 fill-current" />
                </div>
                <span class="font-medium" style="color:#D4C5A9;">4.9 / 5</span>
                <span style="color:#5A5450;">{{ $t('storefront.product.reviewsCount', { count: 1000 }) }}</span>
            </div>

            <!-- Price -->
            <div class="flex flex-wrap items-baseline gap-3 mb-5">
                <div class="text-4xl font-light tracking-tight" style="color:#D4C5A9; font-family:'Cormorant Garamond',serif;">
                ...
                <div v-if="originalPrice && originalPrice > currentPrice" class="px-2.5 py-1 text-xs font-medium" style="background-color:#7A1F0F; color:#FCA5A5; border-radius:1px;">
            ...
            <!-- Demand badge -->
            <div class="flex items-center gap-3 p-3 border" style="background-color:#1A1F2E; border-color:rgba(166,124,82,0.2); border-radius:1px;">
                <div class="relative flex h-2.5 w-2.5">
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full" style="background-color:#A67C52; opacity:0.6;"></span>
                    <span class="relative inline-flex rounded-full h-2.5 w-2.5" style="background-color:#A67C52;"></span>
                </div>
                <span class="text-sm" style="color:#8A8070;">
                    <strong class="font-semibold" style="color:#D4C5A9;">{{ $t('storefront.product.highDemandBadge') }}</strong> {{ $t('storefront.product.viewingRightNow', { count: 15 }) }}
                </span>
            </div>
        </div>

        <!-- Option Selectors -->
        <div v-if="product?.options && product.options.length > 0" class="space-y-5">
            <div v-for="option in product.options" :key="option.id">
                <label class="block text-xs font-medium tracking-[0.2em] uppercase mb-3" style="color:#A67C52;">{{ option.name }}</label>
                ...
                <div v-if="option.displayType === 'dropdown'" class="relative max-w-xs">
                    <select
                        ...
                        style="background-color:#131720; border:1px solid rgba(212,197,169,0.15); color:#D4C5A9; border-radius:1px;"
                        @change="setOption(option.id, ($event.target as HTMLSelectElement).value)"
                    >
                        <option v-for="value in option.values" :key="value.id" :value="value.id" :disabled="isOptionValueUnavailable(option.id, value.id)" style="background-color:#131720;">
                            {{ value.label }} {{ optionValueSuffix(option.id, value.id) }}
                        </option>
                    </select>
                    <div class="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" style="color:#7A7060;">
                ...
                <div v-else-if="option.displayType === 'color'" class="flex flex-wrap gap-3">
                    <button 
                        ...
                        :class="[
                            ...
                            optionValueState(option.id, value.id) === 'unavailable' ? 'opacity-30 cursor-not-allowed' : 'hover:scale-110 cursor-pointer',
                            selectedOptions[option.id] === value.id ? 'ring-2 ring-offset-2 scale-110 shadow-md' : 'ring-1 ring-black/20'
                        ]"
                        :style="{ backgroundColor: value.meta || '#2A3040', '--tw-ring-color': '#A67C52', '--tw-ring-offset-color': '#0E1117' }"
                    ...
                    >
                        <Icon v-if="selectedOptions[option.id] === value.id" name="lucide:check" class="w-4 h-4 text-white drop-shadow-sm" />
                        <div v-if="optionValueState(option.id, value.id) !== 'available'" class="absolute inset-0 flex items-center justify-center">
                            <div class="w-full h-0.5 rotate-45 transform" :class="optionValueState(option.id, value.id) === 'out_of_stock' ? 'bg-red-400' : 'bg-gray-500'" />
                        </div>
                    </button>
                </div>
                ...
                <div v-else class="flex flex-wrap gap-2">
                    <button 
                        ...
                        :style="selectedOptions[option.id] === value.id
                    ...
                    >
                        ...
                        <span v-if="optionValueState(option.id, value.id) !== 'available'" class="ml-2 text-[10px] font-normal">
        ...
        <!-- Mini Description -->
        <p v-if="product?.miniDescription" class="text-lg leading-relaxed" style="color:#8A8070;">
