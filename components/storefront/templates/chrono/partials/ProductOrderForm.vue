...
<template>
  <div>
    <!-- Quantity Selector -->
    <div
      class="flex items-center justify-between p-4 mb-6 border"
      style="background-color:#131720; border-color:rgba(212,197,169,0.12); border-radius:2px;"
    >
      ...
      <div
        class="flex items-center gap-1 p-1 border"
        style="background-color:#0E1117; border-color:rgba(212,197,169,0.1); border-radius:1px;"
      >
        <button
          type="button"
    ...
    >
      <!-- Top accent line -->
      <div
        class="absolute top-0 start-0 end-0 h-px"
        style="background:linear-gradient(to right, #A67C52, rgba(166,124,82,0.1));"
      />
            
      <div class="flex items-center gap-3 mb-6">
        <div
          class="w-10 h-10 flex items-center justify-center border"
          style="background-color:#1A1F2E; border-color:rgba(166,124,82,0.25); border-radius:1px; color:#A67C52;"
        >
          <Icon
        ...
        <div>
          ...
          <span
            class="text-xs"
            style="color:#5A5450;"
          >{{ storefrontContent.productForm.cod.badge }}</span>
        </div>
      </div>
      ...
      >
        <div class="space-y-1.5">
          <label
            class="block text-xs font-medium tracking-[0.2em] uppercase ms-1"
            style="color:#A67C52;"
          >{{ storefrontContent.checkout.form.fullName.label }}</label>
          <input
            v-model="quickForm.fullName"
            type="text"
            :placeholder="storefrontContent.checkout.form.fullName.placeholder"
            class="block w-full h-11 px-4 text-sm placeholder:text-[#3A3530] focus:outline-none transition-all"
            style="background-color:#0E1117; border:1px solid rgba(212,197,169,0.12); color:#E8E0D5; border-radius:1px;"
            :class="{ 'border-[#A67C52]': !quickForm.fullName }"
          >
        </div>
        ...
        <div class="space-y-1.5">
          <label
            class="block text-xs font-medium tracking-[0.2em] uppercase ms-1"
            style="color:#A67C52;"
          >{{ storefrontContent.checkout.form.phone.label }}</label>
          <input
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1.5">
            <label
              class="block text-xs font-medium tracking-[0.2em] uppercase ms-1"
              style="color:#A67C52;"
            >{{ storefrontContent.checkout.form.wilaya.label }}</label>
            <WilayaField
          ...
          <div class="space-y-1.5">
            <label
              class="block text-xs font-medium tracking-[0.2em] uppercase ms-1"
              style="color:#A67C52;"
            >{{ storefrontContent.checkout.form.commune.label }}</label>
            <CommuneField
        ...
        >
          <label
            class="block text-xs font-medium tracking-[0.2em] uppercase ms-1"
            style="color:#A67C52;"
          >{{ storefrontContent.checkout.form.address.label }}</label>
          <input
            v-model="quickForm.address"
            type="text"
            :placeholder="storefrontContent.checkout.form.address.placeholder"
            class="block w-full h-11 px-4 text-sm placeholder:text-[#3A3530] focus:outline-none"
            style="background-color:#0E1117; border:1px solid rgba(212,197,169,0.12); color:#E8E0D5; border-radius:1px;"
          >
        </div>
        <div
        ...
        <div
          ...
          style="background-color:rgba(139,20,20,0.2); border-color:rgba(200,80,80,0.25); color:#FCA5A5; border-radius:1px;"
        >
          {{ orderError }}
        </div>
        ...
        <div
          ...
          class="flex items-center justify-between px-1 py-2 text-sm"
        >
          <span
            class="uppercase tracking-wider text-xs font-bold"
            style="color:#D4C5A9;"
          >{{ t('storefront.clearance.discountLine') }}</span>
          <span
            class="font-medium"
            style="color:#D4C5A9;"
          >-{{ formatAmount(quickOrderClearanceDiscount) }} {{ currencyCode }}</span>
        </div>

        <!-- Total Price -->
        <div
          class="flex items-center justify-between px-4 py-3 border"
          style="background-color:#0E1117; border-color:rgba(212,197,169,0.1); border-radius:1px;"
        >
          <span
          ...
          >{{ formatAmount(totalPrice + (selectedDelivery?.price && selectedDelivery?.price !== 'FREE' && selectedDelivery?.price !== '—' ? Number(selectedDelivery.price) : 0)) }} {{ currencyCode }}</span>
        </div>
        ...
        >
          <div class="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent group-hover:translate-x-full transition-transform duration-700 z-0" />
          <Icon
    ...
    <div class="mt-8">
      <p
        class="text-center text-xs uppercase tracking-[0.2em] mb-4"
        style="color:#4A4540;"
      >
      ...
      <div class="grid grid-cols-3 gap-3">
        <div
          class="flex flex-col items-center text-center gap-2 p-3 border"
        ...
        >
          ...
          <span
            ...
            style="color:#8A8070;"
          >{{ $t('storefront.product.features.delivery') }}</span>
        </div>
        <div
          class="flex flex-col items-center text-center gap-2 p-3 border"
          style="background-color:#131720; border-color:rgba(212,197,169,0.08); border-radius:2px;"
        >
          ...
          <span
            class="text-[10px] font-medium leading-tight"
            style="color:#8A8070;"
          >{{ $t('storefront.product.features.support') }}</span>
        </div>
        <div
          class="flex flex-col items-center text-center gap-2 p-3 border"
          style="background-color:#131720; border-color:rgba(212,197,169,0.08); border-radius:2px;"
        >
          ...
          <span
            class="text-[10px] font-medium leading-tight"
            style="color:#8A8070;"
          >{{ $t('storefront.product.features.securePayment') }}</span>
        </div>
      </div>
    </div>
    ...
    >
      <div
        ...
        style="background-color:#1A1F2E; border:1px solid rgba(166,124,82,0.25); border-radius:2px; color:#E8E0D5;"
      >
        <div
          class="w-8 h-8 flex items-center justify-center shrink-0"
          style="background-color:#A67C52; border-radius:1px; color:#fff;"
        >
          <Icon
            name="lucide:check"
            class="w-4 h-4"
    ...
    >
      <div
        ...
        class="fixed bottom-0 start-0 end-0 z-40 p-4 md:hidden flex items-center justify-between gap-4 border-t"
        style="background-color:#111620; border-color:rgba(166,124,82,0.15);"
      >
        ...
        <button
          type="button"
          :disabled="!canPurchase"
          class="flex-1 h-12 font-medium text-sm tracking-[0.15em] uppercase transition-all flex items-center justify-center gap-2 disabled:opacity-40"
          style="background-color:#A67C52; color:#fff; border-radius:1px;"
          @click="scrollToForm"
        >
          <span>{{ storefrontContent.productForm.cod.submit }}</span>
