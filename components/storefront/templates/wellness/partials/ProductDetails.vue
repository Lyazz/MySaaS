...
<template>
  <div class="flex flex-col animate-fade-in-right space-y-8">
    ...
    <div class="border-b border-stone-200 pb-6">
      <div class="mb-4">
        <span
          class="inline-block px-3 py-1 rounded-full bg-brand-50 text-brand-700 text-xs font-bold tracking-widest uppercase"
        >
          Premium Collection
        </span>
      </div>
      <h1
        class="text-4xl md:text-5xl lg:text-6xl font-wellness font-medium text-stone-900 tracking-tight leading-none mb-6"
      >
        {{ product?.title }}
      </h1>
    ...
    >
      <div v-for="option in product.options" :key="option.id">
        <label
          class="block text-sm font-bold text-stone-800 uppercase tracking-wider mb-3"
          >{{ option.name }}</label
        >
        ...
        <div v-else-if="option.displayType === 'radio'" class="space-y-3">
          ...
          >
            <div class="relative flex items-center justify-center w-5 h-5 me-3">
              ...
              ></div>
              <div
                class="w-3 h-3 rounded-full bg-brand-600 absolute transition-all duration-200"
                :class="
                  selectedOptions[option.id] === value.id
                    ? 'scale-100'
                    : 'scale-0'
