...
<template>
  <div class="min-h-screen bg-wl-paper py-8 lg:py-12 font-wellness relative text-wl-ink">
    ...
    >
      <div
        ...
        class="fixed inset-0 bg-wl-ink/40 z-40 lg:hidden"
    ...
    >
      <aside
        v-if="isFilterDrawerOpen"
        class="fixed inset-y-0 end-0 w-[320px] bg-wl-paper z-50 shadow-2xl overflow-y-auto lg:hidden border-s border-wl-rule"
      >
        <div class="flex items-center justify-between px-6 h-20 bg-wl-card border-b border-wl-ruleStrong">
        ...
        <div class="p-6 space-y-8">
            ...
            <div class="space-y-3">
              ...
              >
                <input
                  ...
                  type="checkbox"
                  :value="cat.id"
                  class="h-4 w-4 rounded-none border-wl-ruleStrong text-wl-ink focus:ring-0 focus:ring-offset-0 bg-wl-card transition-colors"
                >
                <span class="text-sm text-wl-muted group-hover:text-wl-ink transition-colors">{{ categoryDisplayTitle(cat) }}</span>
              </label>
            </div>
          </div>
          ...
          <div class="pt-4 sticky bottom-0 pb-safe bg-wl-paper">
            <button
              class="w-full py-4 bg-wl-ink text-wl-paper wl-label hover:bg-brand-700 transition-colors"
              @click="isFilterDrawerOpen = false"
            >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 lg:mb-12">
        ...
        <div class="flex flex-wrap gap-2 mt-7">
          <button
            class="px-6 py-2.5 bg-wl-ink text-wl-paper wl-label hover:bg-brand-700 transition-colors whitespace-nowrap"
            @click="resetFilters"
          >
      ...
      <div class="flex flex-col lg:flex-row gap-10">
        ...
        <aside class="hidden lg:block w-64 flex-shrink-0 space-y-6 sticky top-28 max-h-[calc(100vh-8rem)] overflow-y-auto pe-1 custom-scrollbar">
          <div class="bg-wl-card border border-wl-rule p-6">
            <!-- Filter Header -->
                ...
                <div class="space-y-3">
                  ...
                  >
                    <input
                      ...
                      type="checkbox"
                      :value="cat.id"
                      class="h-4 w-4 rounded-none border-wl-ruleStrong text-wl-ink focus:ring-0 focus:ring-offset-0 bg-wl-paper transition-colors"
                    >
                    <span class="text-sm text-wl-muted group-hover:text-wl-ink transition-colors">{{ categoryDisplayTitle(cat) }}</span>
                  </label>
                </div>
        ...
        <div class="flex-1 min-w-0">
          ...
          <div v-if="selectedCategories.length > 0" class="flex flex-wrap items-center gap-2 mb-6">
              <div
                ...
                :key="catId"
                class="flex items-center gap-2 px-3 py-1.5 bg-wl-ink text-wl-paper wl-label"
              >
          ...
          <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mb-8 pb-6 border-b border-wl-rule">
            ...
            <button
              class="w-full sm:hidden flex items-center justify-center gap-2 px-6 py-3.5 bg-wl-card border border-wl-ruleStrong text-wl-ink wl-label transition-colors hover:bg-wl-paper"
            ...
            <div class="relative max-w-sm w-full">
              <input
                ...
                class="w-full bg-wl-card border border-wl-rule text-wl-ink text-sm block ps-11 pe-4 py-3 outline-none focus:border-wl-ink transition-colors placeholder:text-wl-muted/60"
              >
            ...
            <div class="hidden sm:flex items-center gap-3 flex-shrink-0">
              ...
              <div class="relative w-48">
                <select
                  v-model="sortOption"
                  class="w-full appearance-none bg-wl-card border border-wl-rule text-sm py-3 ps-4 pe-10 focus:border-wl-ink cursor-pointer transition-colors text-wl-ink outline-none"
                >
              ...
              <div class="flex items-center border border-wl-rule">
                  <button
                      ...
                      :class="viewMode === 'grid' ? 'bg-wl-ink text-wl-paper' : 'bg-wl-card text-wl-muted hover:text-wl-ink'"
                      @click="viewMode = 'grid'"
                  ...
                  <button
                      ...
                      :class="viewMode === 'list' ? 'bg-wl-ink text-wl-paper' : 'bg-wl-card text-wl-muted hover:text-wl-ink'"
                      @click="viewMode = 'list'"
          ...
          <div
            ...
            class="bg-wl-card border border-wl-rule p-16 text-center"
          >
            ...
            <button
              class="mt-8 px-8 py-3.5 bg-wl-ink text-wl-paper wl-label hover:bg-brand-700 transition-colors"
              @click="resetFilters"
            >
              {{ storefrontContent.actions.clearAll }}
            </button>
  ...
  >
      <div v-if="isQuickViewOpen && quickViewProduct" class="wl-root fixed inset-0 z-[60] flex items-center justify-center p-4 font-wellness">
          <div class="absolute inset-0 bg-wl-ink/50" @click="closeQuickView"></div>
          <div class="bg-wl-card border border-wl-ruleStrong shadow-2xl max-w-4xl w-full max-h-[90vh] relative z-10 flex flex-col md:flex-row overflow-hidden">
              <button @click="closeQuickView" class="absolute top-4 end-4 z-20 p-2 bg-wl-card border border-wl-rule hover:bg-wl-paper transition-colors text-wl-muted hover:text-wl-ink">
                  <Icon name="lucide:x" class="w-4 h-4" />
              </button>

              <div class="w-full md:w-1/2 aspect-square md:aspect-auto bg-wl-paper relative flex-shrink-0">
                  <img
              ...
              <div class="w-full md:w-1/2 p-8 md:p-10 flex flex-col overflow-y-auto">
                  <div>
                    <span class="inline-block wl-label text-wl-muted mb-5">{{ storefrontContent.product.inStock }}</span>
                  ...
                  <div class="mt-auto space-y-2">
                      <button class="w-full py-4 bg-wl-ink text-wl-paper wl-label hover:bg-brand-700 transition-colors flex items-center justify-center gap-3">
                        <Icon name="lucide:shopping-bag" class="w-4 h-4" />
                        {{ storefrontContent.actions.addToCart }}
                      </button>
                      <NuxtLink :to="`/product/${quickViewProduct.slug}`" class="block w-full py-4 border border-wl-rule text-wl-muted hover:text-wl-ink hover:border-wl-ink wl-label transition-colors text-center" @click="closeQuickView">
