...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="border-b-4 border-black pb-6">
      <span
        class="inline-block bg-black text-white px-3 py-1 font-mono text-sm uppercase mb-2 tracking-widest"
      >
        {{ product?.category?.title || 'Collection' }}
      </span>
      <h1
        class="font-street text-5xl md:text-7xl leading-none uppercase mb-4 text-black drop-shadow-[4px_4px_0_var(--brand)]"
      >
        {{ product?.title }}
      </h1>
      ...
      <div class="flex items-center gap-4">
        <span
          class="text-3xl font-mono font-bold bg-brand px-2 border-2 border-black shadow-[4px_4px_0_0_#000]"
        >
          {{ formatPrice(currentPrice) }}
        </span>
        <span
          v-if="product?.compareAtPrice"
          class="text-xl text-gray-500 line-through decoration-2 decoration-red-500 font-mono"
        >
    ...
    >
      <div v-for="option in product.options" :key="option.id">
        <label class="block font-street text-2xl uppercase mb-3">{{
          option.name
        }}</label>
        ...
        >
          ...
          >
            ...
            />
            <span v-else class="text-xs font-mono uppercase text-gray-400">{{
              value.label
            }}</span>
            <div
