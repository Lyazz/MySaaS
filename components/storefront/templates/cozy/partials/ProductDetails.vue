...
<template>
  <div class="space-y-6">
    ...
    <!-- Title Section -->
    <div>
      <span class="inline-block px-3 py-1 bg-brand-50 text-brand-500 rounded-full text-xs font-medium mb-3">
        {{ product?.category?.title || 'Collection' }}
      </span>
      <h1 class="font-cozy font-black text-3xl md:text-4xl text-slate-800 leading-tight mb-4">
        {{ product?.title }}
      </h1>
...
<div class="flex items-center gap-4">
    ...
    >
      <div
        ...
        :key="option.id"
      >
        <label class="block text-sm font-medium text-slate-600 mb-3">{{ option.name }}</label>
          ...
          >
            ...
            <div v-if="optionValueState(option.id, value.id) !== 'available'" class="absolute inset-0 flex items-center justify-center">
              <div
                ...
                :class="optionValueState(option.id, value.id) === 'out_of_stock' ? 'bg-red-400' : 'bg-slate-400'"
              />
            </div>
          </button>
        </div>
          ...
          >
            ...
            <label 
              ...
              :class="{
                ...
                'text-red-500': optionValueState(option.id, value.id) === 'out_of_stock',
                'text-slate-700': optionValueState(option.id, value.id) === 'available'
              }"
            >
              {{ value.label }} <span class="text-xs">{{ optionValueSuffix(option.id, value.id) }}</span>
            </label>
          </div>
        </div>
