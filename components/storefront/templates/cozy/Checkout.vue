                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl flex items-center justify-center" :class="`bg-${option.color}-100 text-${option.color}-500`">
                      <CarrierMark :provider="option.provider" :icon="option.icon" :alt="option.providerLabel" class="w-6 h-6" />
                    </div>
                    <div>
        ...
        <div class="lg:col-span-5">
          <div class="bg-white p-8 rounded-[2rem] shadow-soft sticky top-24">
            ...
            <dl class="space-y-3 text-sm mb-8">
              ...
              <div v-if="cartStore.clearanceDiscount > 0" class="flex justify-between text-amber-600">
                <dt>{{ t('storefront.clearance.discountLine') }}</dt>
                <dd class="font-medium text-amber-600">-{{ formatCurrency(cartStore.clearanceDiscount) }}</dd>
              </div>
              <div v-if="selectedDelivery" class="flex justify-between text-slate-500">
