...
<template>
  <div class="bg-[#f8faf9] min-h-screen py-10 font-sans text-slate-600">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-stationery font-bold text-slate-900">
        ...
        <div
          v-if="!cartEnabled"
          class="mt-3 rounded-sm border border-amber-200 bg-amber-50 text-amber-800 text-sm px-4 py-3"
        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 space-y-8">
          <!-- Personal Info -->
          <div class="bg-white p-6 rounded-sm shadow-sm border border-stone-200">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="col-span-2 md:col-span-1 space-y-2">
                ...
                <input
                  ...
                  type="text"
                  class="w-full h-12 rounded-sm border border-stone-300 bg-white px-4 text-slate-900 placeholder:text-slate-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none shadow-sm"
              ...
              <div class="col-span-2 md:col-span-1 space-y-2">
                ...
                <div class="relative">
                  <select
                    v-model="form.wilaya"
                    class="w-full h-12 rounded-sm border border-stone-300 bg-white px-4 text-slate-900 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none appearance-none cursor-pointer shadow-sm"
                  >
              ...
              <div class="col-span-2 md:col-span-1 space-y-2">
                ...
                <CommuneField
                  ...
                  :input-class="'w-full h-12 rounded-sm border border-stone-300 bg-white px-4 text-slate-900 placeholder:text-slate-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none shadow-sm'"
                  :select-class="'w-full h-12 rounded-sm border border-stone-300 bg-white px-4 text-slate-900 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none shadow-sm'"
                />
              </div>
          ...
          <!-- Delivery Options -->
          <div v-if="form.wilaya && form.commune" class="bg-white p-6 rounded-sm shadow-sm border border-stone-200">
            <div class="flex items-center justify-between mb-4">
              ...
              <span class="text-[10px] font-bold text-brand-600 bg-brand-100 px-2 py-1 rounded-sm uppercase">
            ...
            <div class="space-y-3">
              <div 
                ...
                :key="option.id"
                class="cursor-pointer relative rounded-sm p-4 border transition-all duration-300 group hover:scale-[1.005]"
              ...
              >
                <div class="flex items-center gap-4">
                  <!-- Icon -->
                  <div 
                    class="w-14 h-14 rounded-sm flex items-center justify-center flex-shrink-0 transition-all duration-300"
                    :class="form.selectedDeliveryOption === option.id 
                      ? `bg-${option.color}-100` 
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      ...
                      <span class="text-[10px] font-bold px-2 py-0.5 rounded-sm uppercase tracking-wide"
                ...
                <div v-if="option.mode === 'pickup' && option.provider === 'MAYSTRO' && (pickupPointsLoading || stopDeskName || pickupPointsError)" class="mt-3 pt-3 border-t border-stone-100">
          ...
          <div v-else class="bg-white p-6 rounded-sm shadow-sm border border-stone-200 text-center text-sm text-slate-400">
        ...
        <div class="lg:col-span-5">
          <div class="bg-white p-7 rounded-sm shadow-md shadow-stone-200/50 border border-stone-200 sticky top-24">
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
              <h2 class="text-xl font-bold text-slate-900">
                Order Summary
              </h2>
              <div class="flex items-center gap-1.5 text-sm font-semibold text-brand-600 bg-brand-50 px-3 py-1.5 rounded-sm">
            ...
            <div class="space-y-4 mb-6 max-h-80 overflow-y-auto pr-2">
              ...
              >
                <div class="h-16 w-16 bg-slate-100 rounded-sm flex-shrink-0 overflow-hidden relative border border-slate-200">
                  <img
                    v-if="item.image"
                    :src="item.image"
                    :alt="item.title"
            ...
            <!-- Coupon -->
            <div class="bg-gradient-to-br from-brand-50/30 to-brand-100/20 p-5 rounded-sm border border-brand-200/50 mb-6 backdrop-blur-sm">
              <div class="flex justify-between items-center mb-3">
                ...
                <span class="text-[10px] font-bold text-brand-700 bg-brand-200/60 px-2.5 py-1 rounded-sm uppercase tracking-wide">
              ...
              <div class="flex gap-2">
                <input
                  ...
                  class="flex-1 h-11 rounded-sm border border-stone-300 bg-white px-4 text-sm text-slate-900 placeholder:text-slate-400 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none shadow-sm font-medium"
                >
                <button class="px-5 h-11 bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm rounded-sm transition-all duration-200 hover:shadow-lg hover:shadow-brand-200 hover:-translate-y-0.5 active:translate-y-0 whitespace-nowrap">
            ...
            <div
              v-if="errorMessage"
              class="mt-4 rounded-sm border border-red-300 bg-red-50 text-red-800 text-sm px-4 py-3.5 flex items-start gap-3"
            >
            ...
            <button 
              ...
              class="w-full mt-6 bg-slate-900 hover:bg-brand-700 text-white font-bold py-4 px-6 rounded-sm shadow-lg shadow-brand-300/30 transition-all duration-300 hover:shadow-xl hover:shadow-brand-400/40 hover:-translate-y-1 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 flex items-center justify-center gap-2.5 group relative overflow-hidden"
              @click="handleSubmit"
            >
