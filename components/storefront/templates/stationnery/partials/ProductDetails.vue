...
<template>
    <div class="flex flex-col animate-fade-in-right space-y-8">
        <!-- Header -->
        <div>
        <h1 class="text-3xl md:text-4xl lg:text-5xl font-stationery font-bold text-slate-900 tracking-tight leading-tight mb-4">
            {{ product?.title }}
        </h1>
        ...
        <StorefrontSharedCountdownTimer
            ...
            class="mb-4"
        />
<div class="text-4xl font-stationery font-bold text-brand-700 tracking-tight">
            {{ formatPrice(currentPrice) }}
            </div>
            <div
            v-if="(Number(props.product?.price) * 1.2) > 0"
            class="text-lg text-slate-400 line-through decoration-1 decoration-slate-300 font-stationery italic"
            >
        ...
        >
            ...
            <!-- Dropdown Type -->
             <div v-if="option.displayType === 'dropdown'" class="relative max-w-xs">
                <select
                    ...
                    class="block w-full h-11 rounded-sm border border-stone-300 bg-white px-4 text-slate-900 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all duration-200 outline-none appearance-none cursor-pointer shadow-sm"
                >
	                    <option 
	            ...
	            <div v-else-if="option.displayType === 'color'" class="flex flex-wrap gap-3">
	                <button 
	                    ...
	                    type="button"
	                    :class="[
	                        'w-10 h-10 rounded-sm flex items-center justify-center transition-all duration-200 relative border border-stone-200',
             ...
             <div v-else-if="option.displayType === 'image'" class="flex flex-wrap gap-3">
                 <button 
                    ...
                    type="button"
                    :class="[
                        'w-16 h-16 rounded-sm border transition-all duration-200 overflow-hidden relative',
                         optionValueState(option.id, value.id) === 'unavailable'
            ...
            <button 
                v-for="value in option.values" 
                :key="value.id"
                class="px-4 py-2 rounded-sm text-sm font-medium border transition-all duration-200 relative overflow-hidden"
                :class="[
                    optionValueState(option.id, value.id) === 'unavailable'
