    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-7 space-y-8">
          ...
          <!-- Delivery Mode -->
          <div class="bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
            <h3 class="text-sm font-bold text-slate-900 mb-4 uppercase tracking-wide">
              Delivery mode
            </h3>
            <p class="text-xs text-slate-500 mb-6 font-medium">
              Choose home delivery, pickup point, or store pickup.
            </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              >
                <div class="flex items-center justify-between mb-3">
                  ...
                  <span
                    class="block w-4 h-4 rounded-full border-2 transition-colors duration-300"
                    :class="form.deliveryMode === 'home' ? 'border-brand-600 bg-brand-600' : 'border-slate-300'"
                  />
                </div>
              ...
              >
                <div class="flex items-center justify-between mb-3">
                  ...
                  <span
                    class="block w-4 h-4 rounded-full border-2 transition-colors duration-300"
                    :class="form.deliveryMode === 'pickup' ? 'border-brand-600 bg-brand-600' : 'border-slate-300'"
                  />
                </div>
                <h4 class="font-bold text-slate-900 text-sm mb-1">
                  Pickup Point
              ...
              >
                <div class="flex items-center justify-between mb-3">
                  ...
                  <span
                    class="block w-4 h-4 rounded-full border-2 transition-colors duration-300"
                    :class="form.deliveryMode === 'store' ? 'border-brand-600 bg-brand-600' : 'border-slate-300'"
                  />
                </div>
                <h4 class="font-bold text-slate-900 text-sm mb-1">
                  Store Pickup
                ...
                <p class="text-xs text-slate-500 leading-relaxed">
                  Pick up at our store in Ouled Fayet.
                </p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white p-6 rounded-3xl shadow-soft border border-slate-100">
            ...
            <textarea
              v-model="form.notes"
              rows="3"
              class="w-full rounded-2xl border border-slate-200 bg-white p-4 text-slate-900 font-medium placeholder:text-slate-400 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 transition-all duration-300 outline-none shadow-sm resize-none"
              placeholder="Delivery instructions, time slots..."
            />
          </div>
            ...
            <label class="flex items-center gap-4 p-4 border-2 border-slate-100 rounded-2xl cursor-pointer hover:border-brand-500 bg-slate-50/50 hover:bg-white transition-all duration-300 group">
              <div class="relative flex items-center justify-center w-6 h-6">
                <input
                  type="radio"
                  checked
                  class="peer appearance-none h-5 w-5 border-2 border-slate-300 rounded-full checked:border-brand-600 checked:bg-brand-600 transition-colors cursor-pointer"
                >
        ...
        <div class="lg:col-span-5">
          <div class="bg-white p-6 rounded-2xl shadow-soft border border-slate-100 sticky top-24">
            ...
            <div class="bg-brand-50/50 p-4 rounded-xl border border-brand-100 mb-6">
              ...
              <div class="flex gap-2">
                <input
                  ...
                  class="flex-1 rounded-lg border-slate-200 text-sm py-2 px-3 focus:ring-brand-500 focus:border-brand-500"
                >
                <button class="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold text-sm rounded-lg transition-colors">
                  Apply
                </button>
              </div>
            </div>
            ...
            <!-- Checkout Button -->
            <button 
              :disabled="!cartStore.hasItems || !cartEnabled"
              ...
              @click="handleSubmit"
            >
              {{ !cartEnabled ? 'Checkout disabled' : 'Place order' }}
            </button>
          </div>
        </div>
      </div>
