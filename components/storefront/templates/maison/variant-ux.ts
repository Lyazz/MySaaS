...
export const isVariantActive = (variant: ProductVariantLike): boolean =>
...
const variantMatchesSelection = (
  ...
  const variantValueIds = new Set(
    (variant.optionValues ?? []).map((ov) => ov.optionValueId)
  );
...
export const getOptionValueState = (params: {
  ...
  const candidateSelection: SelectedOptions = {
    ...selectedOptions,
    [optionId]: valueId,
  };

  const matchingActive = variants.filter(
    (v) =>
      isVariantActive(v) &&
      variantMatchesSelection(product, v, candidateSelection)
  );
...
export const getPreferredInitialSelection = (
  product: ProductLike
): SelectedOptions => {
  if ((product.options ?? []).length === 0) return {};

  // Variant products must start unselected so the customer explicitly chooses options.
  return {};
};
...
export const findBestVariantForSelection = (params: {
  ...
  const candidates = variants.filter(
    (v) =>
      isVariantActive(v) && variantMatchesSelection(product, v, selectedOptions)
  );
