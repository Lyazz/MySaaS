            ...
            <form class="space-y-5" @submit.prevent="handleOrderSubmit">
                ...
                <div v-if="quickForm.wilaya && quickForm.commune" class="space-y-3 mt-6">
                    ...
                    >
                        ...
                        >
                            <CarrierMark :provider="option.provider" :icon="option.icon" :alt="option.providerLabel" class="w-7 h-7 transition-colors duration-300" :class="quickForm.selectedDeliveryOption === option.id ? `text-${option.color}-600` : 'text-[#9C8B82] group-hover:text-[#5C4A44]'" />
                        </div>
