...
<template>
  <div class="bg-[#FAF3EA] min-h-screen py-8 lg:py-12 relative">
    ...
    >
      <div
        ...
        class="fixed inset-0 bg-[#2E1E20]/50 z-40 lg:hidden"
    ...
    >
      <aside
        v-if="isFilterDrawerOpen"
        class="fixed inset-y-0 end-0 w-[300px] bg-[#FFFDF9] z-50 shadow-2xl p-6 overflow-y-auto lg:hidden"
      >
        <div class="flex items-center justify-between mb-6">
          ...
          <button
            class="p-2 -me-2 text-[#6B5850] hover:text-[#2E1E20]"
            @click="isFilterDrawerOpen = false"
          >
        ...
        <div class="space-y-8">
          ...
          <div>
            ...
            >
              <h4 class="font-bold text-[#2E1E20] text-xs uppercase tracking-wider">
              ...
              >
                <div class="relative flex items-center">
                  <input
                    ...
                    type="checkbox"
                    :value="cat.id"
                    class="peer h-5 w-5 rounded border-[#C9A24B]/50 text-brand-600 focus:ring-brand-500 transition-all checked:bg-brand-600 checked:border-transparent"
                  >
                </div>
                <span class="text-base text-[#5C4A44] group-hover:text-brand-700 transition-colors">{{ categoryDisplayTitle(cat) }}</span>
              </label>
            </div>
          </div>

          <div>
            <h4 class="font-bold text-[#2E1E20] mb-4 text-xs uppercase tracking-wider">
          ...
          <!-- Apply Button Mobile -->
          <div class="pt-8 mt-4 sticky bottom-0 bg-[#FFFDF9] pb-safe">
            <button
              class="w-full py-3 bg-brand-600 text-white font-bold rounded-2xl shadow-lg active:scale-95 transition-transform"
              @click="isFilterDrawerOpen = false"
            >
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-8 lg:mb-10">
        ...
        <h1 class="text-3xl md:text-4xl font-bold text-[#2E1E20] tracking-tight">
          {{ pageTitle }}
        </h1>
      ...
      <div class="flex flex-col lg:flex-row gap-10">
        ...
        <aside class="hidden lg:block w-64 flex-shrink-0 space-y-8 bg-[#FFFDF9] p-6 pt-8 rounded-tl-[48px] rounded-tr-lg rounded-br-lg rounded-bl-lg border border-[#C9A24B]/35 sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto pr-2 custom-scrollbar shadow-sm">
          ...
          <div class="flex items-center justify-between mb-4 border-b border-[#C9A24B]/25 pb-4">
            ...
            <button
              class="text-xs font-semibold text-brand-700 hover:text-brand-800 uppercase tracking-wide"
              @click="resetFilters"
            >
              ...
              >
                <div class="relative flex items-center">
                  <input
                    ...
                    type="checkbox"
                    :value="cat.id"
                    class="peer h-4 w-4 rounded border-[#C9A24B]/50 text-brand-600 focus:ring-brand-500 transition-all checked:bg-brand-600 checked:border-transparent"
                  >
                </div>
                <span class="text-sm text-[#5C4A44] group-hover:text-brand-700 transition-colors">{{ categoryDisplayTitle(cat) }}</span>
              </label>
            </div>
          </div>
          ...
          <!-- Best Sellers Widget -->
          <div class="pt-8 border-t border-[#C9A24B]/25">
            <h4 class="font-bold text-[#2E1E20] mb-4 text-xs uppercase tracking-wider">
            ...
            <div class="space-y-4">
              ...
              >
                <div class="w-16 h-16 bg-[#F3E7D8] rounded-xl overflow-hidden flex-shrink-0">
                  <img
                ...
                <div>
                  <h5 class="text-sm font-bold text-[#2E1E20] line-clamp-2 group-hover:text-brand-700 transition-colors">
                    {{ p.title }}
                  </h5>
                  <span class="text-xs font-semibold text-[#6B5850]">{{ formatCurrency(p.price) }}</span>
                </div>
              </NuxtLink>
            </div>
          </div>
        ...
        <div class="flex-1">
          ...
          <div v-if="selectedCategories.length > 0" class="flex flex-wrap gap-2 mb-6">
              ...
              <button @click="resetFilters" class="text-sm text-[#6B5850] hover:text-brand-700 underline underline-offset-2">
          ...
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 pb-4 border-b border-[#C9A24B]/20">
            ...
            <div class="hidden sm:flex items-center gap-3 w-full sm:w-auto">
              <span class="text-sm text-[#6B5850] whitespace-nowrap">{{ storefrontContent.shop.sortBy }}</span>
          ...
          >
            ...
            <button
              class="mt-6 px-6 py-2 bg-[#2E1E20] text-white rounded-full hover:bg-brand-700 transition-colors"
              @click="resetFilters"
            >
              {{ storefrontContent.actions.clearAll }}
            </button>
  ...
  >
      <div v-if="isQuickViewOpen && quickViewProduct" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div class="bg-[#FFFDF9] rounded-tl-[56px] rounded-tr-2xl rounded-br-[56px] rounded-bl-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative z-10 flex flex-col md:flex-row overflow-hidden border border-[#C9A24B]/30">
              ...
              <div class="w-full md:w-1/2 aspect-square md:aspect-auto bg-[#F3E7D8] relative">
                  <img
              ...
              <div class="w-full md:w-1/2 p-8 md:p-12 flex flex-col">
                  <div>
                    ...
                    <h2 class="text-3xl font-bold text-[#2E1E20] mb-2">{{ quickViewProduct.title }}</h2>
                    <div class="text-2xl font-bold text-[#2E1E20] mb-6 flex items-center gap-3">
                        {{ formatCurrency(quickViewProduct.price) }}
                    </div>
                    <p class="text-[#5C4A44] leading-relaxed mb-8">
                        {{ quickViewProduct.description || storefrontContent.product.descriptionFallback }}
                    </p>
                  </div>

                  <div class="mt-auto space-y-4">
                      <button class="w-full py-4 bg-[#2E1E20] text-white font-bold rounded-2xl hover:bg-brand-700 transition-colors shadow-lg active:scale-95 flex items-center justify-center gap-2">
                        <Icon name="lucide:banknote" class="w-5 h-5" />
                        {{ storefrontContent.actions.addToCart }}
                      </button>
                      <NuxtLink :to="`/product/${quickViewProduct.slug}`" class="block w-full py-4 border border-[#C9A24B]/40 text-[#2E1E20] font-bold rounded-2xl hover:bg-[#FAF3EA] transition-colors text-center" @click="closeQuickView">
