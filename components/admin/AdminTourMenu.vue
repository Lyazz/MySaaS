<template>
 ...
 v-if="!sidebarOpen"
 type="button"
 class="w-full flex items-center justify-center p-2 rounded-lg transition-colors text-muted"
...
>
 ...
 v-else
 type="button"
 class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg transition-colors text-start text-muted"
...
>
      ...
      <span class="text-xs font-medium flex-1 truncate">{{ t('admin.tours.menuLabel') }}</span>
 ...
 :key="tour.id"
 type="button"
 class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-start transition-colors group text-secondary"
...
>
          ...
          <span class="text-mini flex-1 truncate">{{ tour.label }}</span>
          <Icon name="lucide:play" class="w-3 h-3 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-brand" />
        </button>
      </div>
    </Transition>
  </div>
